...
static bool _mongocrypt_marking_parse_fle1_placeholder(const bson_t* in,
...
static bool _mongocrypt_marking_parse_fle2_placeholder(const bson_t* in,
                                                       _mongocrypt_marking_t* out,
                                                       mongocrypt_status_t* status) {
    BSON_ASSERT_PARAM(in);
    BSON_ASSERT_PARAM(out);
...
#define DERIVE_TOKEN_IMPL(Name)                                                                  \
    static bool _fle2_derive_##Name##_token(_mongocrypt_crypto_t* crypto,                        \
        ...
        _mongocrypt_buffer_copy_to(                                                              \
            mc_##Name##DerivedFromDataTokenAndCounter_get(fromTokenAndCounter), out);            \
...
static bool _fle2_derive_serverDerivedFromDataToken(
...
static bool _fle2_placeholder_aes_aead_encrypt(
...
static bool _get_tokenKey(_mongocrypt_key_broker_t* kb,
    ...
    if (indexKey.len != MONGOCRYPT_KEY_LEN) {
        CLIENT_ERR("invalid indexKey, expected len=%" PRIu32 ", got len=%" PRIu32,
                   MONGOCRYPT_KEY_LEN,
                   indexKey.len);
    ...
    if (!_mongocrypt_buffer_copy_from_data_and_size(
            tokenKey, indexKey.data + (2 * MONGOCRYPT_TOKEN_KEY_LEN), MONGOCRYPT_TOKEN_KEY_LEN)) {
    ...
    *ret = (_FLE2EncryptedPayloadCommon_t){{0}};
    ...
    if (kb->crypt->opts.use_fle2_v2) {
        ...
        ret->serverTokenDerivationLevel1Token =
            mc_ServerTokenDerivationLevel1Token_new(crypto, &ret->tokenKey, status);
...
static bool _mongocrypt_fle2_placeholder_to_insert_update_common_v1(
    _mongocrypt_key_broker_t* kb,
    ...
    *contentionFactor = 0;
    if (placeholder->maxContentionCounter > 0) {
        ...
        if (!_mongocrypt_random_int64(
                crypto, placeholder->maxContentionCounter + 1, contentionFactor, status)) {
            goto fail;
        }
    }
    ...
    _mongocrypt_buffer_copy_to(
        mc_ServerDataEncryptionLevel1Token_get(common->serverDataEncryptionLevel1Token),
 ...
 */
static bool _mongocrypt_fle2_placeholder_to_insert_update_ciphertext_v1(
    _mongocrypt_key_broker_t* kb,
    ...
    if (!_mongocrypt_fle2_placeholder_to_insert_update_common_v1(
            kb, &payload, &contentionFactor, &common, placeholder, &placeholder->v_iter, status)) {
        goto fail;
    }
...
static bool _mongocrypt_fle2_placeholder_to_insert_update_common(
    _mongocrypt_key_broker_t* kb,
    ...
    if (placeholder->maxContentionCounter > 0) {
         ...
         * placeholder->maxContentionCounter] */
        if (!_mongocrypt_random_int64(
                crypto, placeholder->maxContentionCounter + 1, &out->contentionFactor, status)) {
            goto fail;
        }
    }
 ...
 *  k: contentionFactor}
 */
static bool _mongocrypt_fle2_placeholder_to_insert_update_ciphertext(
    _mongocrypt_key_broker_t* kb,
    _mongocrypt_marking_t* marking,
    _mongocrypt_ciphertext_t* ciphertext,
    mongocrypt_status_t* status) {
    ...
    if (!_mongocrypt_fle2_placeholder_to_insert_update_common(
            kb, &payload, &common, placeholder, &placeholder->v_iter, status)) {
        goto fail;
    }
...
// on error.
static mc_edges_t* get_edges(mc_FLE2RangeInsertSpec_t* insertSpec,
                             size_t sparsity,
                             size_t trimFactor,
                             mongocrypt_status_t* status) {
    ...
    if (value_type == BSON_TYPE_INT32) {
        return mc_getEdgesInt32(
            (mc_getEdgesInt32_args_t){.value = bson_iter_int32(&insertSpec->v),
        ...
        return mc_getEdgesInt64(
            (mc_getEdgesInt64_args_t){.value = bson_iter_int64(&insertSpec->v),
        ...
        return mc_getEdgesInt64(
            (mc_getEdgesInt64_args_t){.value = bson_iter_date_time(&insertSpec->v),
        ...
        mc_getEdgesDecimal128_args_t args = {
            .value = value, .sparsity = sparsity, .trimFactor = trimFactor};
        if (insertSpec->precision.set) {
 ...
 *      ...]}
 */
static bool _mongocrypt_fle2_placeholder_to_insert_update_ciphertextForRange_v1(
    _mongocrypt_key_broker_t* kb,
    _mongocrypt_marking_t* marking,
    _mongocrypt_ciphertext_t* ciphertext,
    mongocrypt_status_t* status) {
    ...
    if (!_mongocrypt_fle2_placeholder_to_insert_update_common_v1(
            kb, &payload, &contentionFactor, &common, &marking->fle2, &insertSpec.v, status)) {
        goto fail;
    }
    ...
    // g:= array<EdgeTokenSet>
    {
        ...
        edges = get_edges(
            &insertSpec, (size_t)placeholder->sparsity, (size_t)placeholder->trimFactor, status);
        if (!edges) {
            goto fail;
        }
 ...
 *      ...]}
 */
static bool _mongocrypt_fle2_placeholder_to_insert_update_ciphertextForRange(
    _mongocrypt_key_broker_t* kb,
    _mongocrypt_marking_t* marking,
    _mongocrypt_ciphertext_t* ciphertext,
    mongocrypt_status_t* status) {
    ...
    if (!_mongocrypt_fle2_placeholder_to_insert_update_common(
            kb, &payload, &common, &marking->fle2, &insertSpec.v, status)) {
        goto fail;
    }
    ...
    switch (bsonType) {
        ...
        case BSON_TYPE_DECIMAL128: {
            ...
            if (findSpec->edgesInfo.value.precision.set) {
                ...
                args.max =
                    OPT_MC_DEC128(mc_dec128_from_bson_iter(&findSpec->edgesInfo.value.indexMax));
                args.precision = findSpec->edgesInfo.value.precision;
            }
            return mc_getMincoverDecimal128(args, status);
#else  // ↑↑↑↑↑↑↑↑ With Decimal128 / Without ↓↓↓↓↓↓↓↓↓↓
            CLIENT_ERR(
                "FLE2 find is not supported for Decimal128: libmongocrypt "
        ...
        default:
            CLIENT_ERR("FLE2 find is not supported for type: %s", mc_bson_type_to_string(bsonType));
            return NULL;
    }
}
 ...
 *  g: [{d: EDC, s: ESC, c: ECC}, ...]}
 */
static bool _mongocrypt_fle2_placeholder_to_find_ciphertextForRange_v1(
    _mongocrypt_key_broker_t* kb,
    _mongocrypt_marking_t* marking,
    _mongocrypt_ciphertext_t* ciphertext,
    mongocrypt_status_t* status) {
 ...
 */
static bool _mongocrypt_fle2_placeholder_to_find_ciphertextForRange(
    _mongocrypt_key_broker_t* kb,
    _mongocrypt_marking_t* marking,
    _mongocrypt_ciphertext_t* ciphertext,
    mongocrypt_status_t* status) {
...
static bool _mongocrypt_fle2_placeholder_to_FLE2UnindexedEncryptedValue(
    _mongocrypt_key_broker_t* kb,
    _mongocrypt_marking_t* marking,
    _mongocrypt_ciphertext_t* ciphertext,
    mongocrypt_status_t* status) {
    ...
    switch (marking->algorithm) {
        case MONGOCRYPT_ENCRYPTION_ALGORITHM_DETERMINISTIC:
            ...
            ret = _mongocrypt_calculate_deterministic_iv(
                kb->crypt->crypto, &key_material, &plaintext, &associated_data, &iv, status);
            if (!ret) {
                goto fail;
            }
