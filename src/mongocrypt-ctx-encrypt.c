...
static bool _fle2_append_encryptionInformation(const mongocrypt_ctx_t* ctx,
    ...
    if (!BSON_APPEND_DOCUMENT_BEGIN(&schema_bson, ns, &encrypted_field_config_bson)) {
        CLIENT_ERR(
            "unable to begin appending 'encryptedFieldConfig' to "
            "'encryptionInformation'.'schema'");
        return false;
    }

    if (!_fle2_append_encryptedFieldConfig(
            ctx, &encrypted_field_config_bson, encryptedFieldConfig, coll_name, status)) {
        return false;
    }

    if (!bson_append_document_end(&schema_bson, &encrypted_field_config_bson)) {
        CLIENT_ERR(
            "unable to end appending 'encryptedFieldConfig' to "
            "'encryptionInformation'.'schema'");
        return false;
    }
    ...
    if (deleteTokens != NULL) {
        ...
        if (!BSON_APPEND_DOCUMENT_BEGIN(
                &encryption_information_bson, "deleteTokens", &delete_tokens_bson)) {
            CLIENT_ERR(
                "unable to begin appending 'deleteTokens' to "
        ...
        if (!bson_append_document_end(&encryption_information_bson, &delete_tokens_bson)) {
            CLIENT_ERR(
                "unable to end appending 'deleteTokens' to "
                "'encryptionInformation'");
            return false;
        }
    }
...
static bool _fle2_insert_encryptionInformation(const mongocrypt_ctx_t* ctx,
    ...
    if (0 != strcmp(cmd_name, "explain") || cmd_target == MC_TO_MONGOCRYPTD) {
        ...
        if (!_fle2_append_encryptionInformation(
                ctx, cmd, ns, encryptedFieldConfig, deleteTokens, coll_name, status)) {
            goto fail;
        }
        bson_destroy(&out);
        goto success;
    ...
    if (!_fle2_append_encryptionInformation(
            ctx, &explain, ns, encryptedFieldConfig, deleteTokens, coll_name, status)) {
        goto fail;
    }
...
static bool _set_schema_from_collinfo(mongocrypt_ctx_t* ctx, bson_t* collinfo) {
    ...
    /* Disallow views. */
    if (bson_iter_init_find(&iter, collinfo, "type") && BSON_ITER_HOLDS_UTF8(&iter) &&
        bson_iter_utf8(&iter, NULL) && 0 == strcmp("view", bson_iter_utf8(&iter, NULL))) {
    ...
    if (bson_iter_find_descendant(&iter, "options.encryptedFields", &iter)) {
        if (!BSON_ITER_HOLDS_DOCUMENT(&iter)) {
            return _mongocrypt_ctx_fail_w_msg(ctx,
                                              "options.encryptedFields is not a BSON document");
        }
        ...
        if (!_mongocrypt_buffer_to_bson(&ectx->encrypted_field_config, &efc_bson)) {
            return _mongocrypt_ctx_fail_w_msg(ctx,
                                              "unable to create BSON from encrypted_field_config");
        }
...
static bool _fle2_mongo_op_markings(mongocrypt_ctx_t* ctx, bson_t* out) {
    ...
    if (!_fle2_insert_encryptionInformation(ctx,
                                            ...
                                            NULL /* deleteTokens */,
                                            ectx->coll_name,
                                            ctx->crypt->csfle.okay ? MC_TO_CSFLE
                                                                   : MC_TO_MONGOCRYPTD,
                                            ctx->status)) {
        return _mongocrypt_ctx_fail(ctx);
    }
...
static bool _collect_key_from_marking(void* ctx,
...
static bool _mongo_feed_markings(mongocrypt_ctx_t* ctx, mongocrypt_binary_t* in) {
    ...
    if (!bson_iter_recurse(&iter, &iter)) {
        return _mongocrypt_ctx_fail_w_msg(ctx,
                                          "malformed marking, could not recurse into 'result'");
    }
 ...
 */
static bool _add_dollar_db(const char* cmd_name,
                           bson_t* cmd,
                           const char* db_name,
                           mongocrypt_status_t* status) {
    bson_t out = BSON_INITIALIZER;
...
#define CHECK_CSFLE_ERROR(Func, FailLabel)                                          \
    ...
    uint8_t* marked_bson = csfle.analyze_query(
        qa, bson_get_data(&cmd), ectx->ns, (uint32_t)strlen(ectx->ns), &marked_bson_len, status);
...
static bool _marking_to_bson_value(void* ctx,
                                   _mongocrypt_marking_t* marking,
                                   bson_value_t* out,
                                   mongocrypt_status_t* status) {
...
static bool _replace_marking_with_ciphertext(void* ctx,
                                             _mongocrypt_buffer_t* in,
                                             bson_value_t* out,
                                             mongocrypt_status_t* status) {
    _mongocrypt_marking_t marking;
...
static bool _check_for_payload_requiring_encryptionInformation(void* ctx,
                                                               _mongocrypt_buffer_t* in,
                                                               mongocrypt_status_t* status) {
    bool* out = (bool*)ctx;
...
static bool _fle2_finalize(mongocrypt_ctx_t* ctx, mongocrypt_binary_t* out) {
    ...
    if (command_needs_deleteTokens(ctx, command_name)) {
        deleteTokens =
            generate_delete_tokens(ctx->crypt->crypto, &ctx->kb, &ectx->efc, ctx->status);
        if (!deleteTokens) {
            bson_destroy(&converted);
            return _mongocrypt_ctx_fail(ctx);
        }
    ...
    if (!_fle2_append_compactionTokens(
            ctx->crypt->crypto, &ctx->kb, &ectx->efc, command_name, &converted, ctx->status)) {
        bson_destroy(&converted);
        return _mongocrypt_ctx_fail(ctx);
    }
...
static bool FLE2RangeFindDriverSpec_to_ciphertexts(mongocrypt_ctx_t* ctx,
    ...
    // Parse FLE2RangeFindDriverSpec.
    {
        ...
        if (!mc_FLE2RangeFindDriverSpec_to_placeholders(
                ...
                &ctx->opts.key_id,
                _mongocrypt_buffer_empty(&ctx->opts.index_key_id) ? &ctx->opts.key_id
...
static bool _fle2_finalize_explicit(mongocrypt_ctx_t* ctx, mongocrypt_binary_t* out) {
    ...
    if (ctx->opts.query_type.set) {
        switch (ctx->opts.query_type.value) {
            ...
            case MONGOCRYPT_QUERY_TYPE_EQUALITY:
                ...
                break;
            default:
                _mongocrypt_ctx_fail_w_msg(ctx, "Invalid value for EncryptOpts.queryType");
                goto fail;
        }
    } else {
    ...
    if (ctx->opts.rangeopts.set) {
        ...
        marking.fle2.trimFactor = ctx->opts.rangeopts.value.trimFactor;
...
static bool _fle2_try_encrypted_field_config_from_map(mongocrypt_ctx_t* ctx) {
    ...
    if (!_mongocrypt_buffer_to_bson(&crypt->opts.encrypted_field_config_map,
        ...
        return _mongocrypt_ctx_fail_w_msg(ctx,
                                          "unable to convert encrypted_field_config_map to BSON");
    }
...
static bool _try_schema_from_cache(mongocrypt_ctx_t* ctx) {
    ...
    if (!_mongocrypt_cache_get(
            &ctx->crypt->cache_collinfo, ectx->ns /* null terminated */, (void**)&collinfo)) {
...
static bool _permitted_for_encryption(bson_iter_t* iter,
...
static bool explicit_encrypt_init(mongocrypt_ctx_t* ctx, mongocrypt_binary_t* msg) {
    ...
    /* Error if any mutually exclusive FLE 1 and FLE 2 options are set. */
    {
        ...
        if (ctx->opts.key_alt_names != NULL) {
            ...
            if (ctx->opts.rangeopts.set) {
                return _mongocrypt_ctx_fail_w_msg(ctx,
                                                  "cannot set both key alt name and range opts");
            }
        }
    ...
    if (ctx->opts.rangeopts.set && ctx->opts.index_type.set) {
        ...
        if (ctx->opts.index_type.value == MONGOCRYPT_INDEX_TYPE_EQUALITY) {
            return _mongocrypt_ctx_fail_w_msg(ctx,
                                              "cannot set range opts with equality index type");
        }
    }

    if (ctx->opts.contention_factor.set &&
        !mc_validate_contention(ctx->opts.contention_factor.value, ctx->status)) {
        return _mongocrypt_ctx_fail(ctx);
    }
    ...
    if (ctx->opts.rangeopts.set &&
        !mc_validate_sparsity(ctx->opts.rangeopts.value.sparsity, ctx->status)) {
        return _mongocrypt_ctx_fail(ctx);
    }
...
bool mongocrypt_ctx_explicit_encrypt_init(mongocrypt_ctx_t* ctx, mongocrypt_binary_t* msg) {
    ...
    if (ctx->opts.query_type.set &&
        ...
        return _mongocrypt_ctx_fail_w_msg(
            ctx, "Encrypt may not be used for range queries. Use EncryptExpression.");
    }
    return true;
}
...
bool mongocrypt_ctx_explicit_encrypt_expression_init(mongocrypt_ctx_t* ctx,
    ...
    if (!ctx->opts.query_type.set ||
        ctx->opts.query_type.value != MONGOCRYPT_QUERY_TYPE_RANGEPREVIEW) {
        return _mongocrypt_ctx_fail_w_msg(ctx,
                                          "EncryptExpression may only be used for range queries.");
    }
    return true;
}

static bool _check_cmd_for_auto_encrypt(mongocrypt_binary_t* cmd,
                                        bool* bypass,
                                        char** collname,
                                        mongocrypt_status_t* status) {
    bson_t as_bson;
...
static bool needs_ismaster_check(mongocrypt_ctx_t* ctx) {
    ...
    return using_mongocryptd &&
        (0 == strcmp(ectx->cmd_name, "create") || 0 == strcmp(ectx->cmd_name, "createIndexes"));
}
...
bool mongocrypt_ctx_encrypt_init(mongocrypt_ctx_t* ctx,
    ...
    if (!ectx->coll_name) {
        return _mongocrypt_ctx_fail_w_msg(
            ctx, "unexpected error: did not bypass or error but no collection name");
    }
