...
bool mc_RangeOpts_parse(mc_RangeOpts_t* ro, const bson_t* in, mongocrypt_status_t* status) {
    bson_iter_t iter;
    bool has_min = false, has_max = false, has_sparsity = false, has_trimFactor = false,
         has_precision = false;
    ...
    *ro = (mc_RangeOpts_t){0};
    ...
    while (bson_iter_next(&iter)) {
        ...
        if (!BSON_ITER_HOLDS_INT64(&iter)) {
            CLIENT_ERR("%sExpected int64 for trimFactor, got: %s",
                       error_prefix,
                       mc_bson_type_to_string(bson_iter_type(&iter)));
            return false;
        };
        ro->trimFactor = bson_iter_int64(&iter);
        END_IF_FIELD
